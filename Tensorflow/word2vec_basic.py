...
def word2vec_basic(log_dir):
    ...
    def mayb_download(filename,expected_bytes):
        local_filename = os.path.join(gettempdir(),filename)
        if not os.path.exists(local_filename):
            local_filename,_ = urllib.request.urlretrieve(url+filename,local_filename)
        stainfo = os.stat(local_filename)
    ...
    filename = mayb_download('text8.zip',31344016)
    def read_data(filename):
        with zipfile.ZipFile(filename) as f:
            data = tf.compat.as_str(f.read(f.namelist()[0])).split()
        return data
    vocabulary = read_data(filename)
    print("Dta size",len(vocabulary))
    ...
    # step3: Function to geenerate a training batch for the skip-window
    ...
    try:
        ...
        plot_only = 500
        print("#############",np.any(np.isnan(final_embeddings)))
        print("#############", np.any(np.isfinite(final_embeddings)))
